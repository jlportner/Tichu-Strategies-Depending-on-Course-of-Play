...
If in Category (1) the willingness to take risks is 8.5, the team should announce a 100$\%$ Tichu. With 4 it should have no effect.  In between it runs linear, as the function shows [\ref{fig:10}].
...
Due to rounding errors in python, we observed that Team 2 won more games than Team 1. 
...
We also created a method in the Solution class to switch the teams in the simulation. This was necessary to reduce runtime. 

\subsection{Optimal strategy in Exchange Stage}

First of all, we will look at the outcome of the Exchange Stage under different circumstances. But which outcome is the best for a team? We try to achieve a maximal point value in three different ways by considering both of traded cards:

\paragraph{Maximum}   We consider only the value of the highest of both traded cards, as this card could contribute towards winning a Tichu.
\paragraph{Square Addition} The sum of both squares of both cards is taken - this method considers both card values.
\paragraph{Square Diffrence} We take the difference of both squared card values and attempt to minimize this as it may be good to have balanced players.
\\
\\
